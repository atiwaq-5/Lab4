...
Notes:
- Uses only single-line shell writes inside namespaces (printf), no heredocs.
- Assumes required packages already installed on the host VM: bind9, bind9utils, unbound, swaks, python3, (optional: opendkim, opendkim-tools, python3-dkim/"dkim")
- Mininet namespaces share the same filesystem; packages installed on the VM are visible to hosts.
"""
import os, time, pprint, re, shlex
...
def dnssec_sign_and_client_validate(net, zone_file='/root/zones/db.example.com.good', zone_name='example.com', dns_host='dns', client_host='h1'):
    ...
    kk=_run(dns,'bash -lc "command -v /usr/sbin/dnssec-keygen || echo MISSING"').strip()
    sz=_run(dns,'bash -lc "command -v /usr/sbin/dnssec-signzone || echo MISSING"').strip()
    ...
    if not has_keys:
        _run(dns,'bash -lc "cd $(dirname %s) && /usr/sbin/dnssec-keygen -a RSASHA256 -b 2048 -n ZONE -f KSK %s"' % (shlex.quote(zone_file), shlex.quote(zone_name)))
        _run(dns,'bash -lc "cd $(dirname %s) && /usr/sbin/dnssec-keygen -a RSASHA256 -b 2048 -n ZONE %s"' % (shlex.quote(zone_file), shlex.quote(zone_name)))
    # pick keys
    ...
    _run(dns,'bash -lc "cd $(dirname %s) && /usr/sbin/dnssec-signzone -o %s -N INCREMENT -k %s %s %s >/tmp/dnssec_sign.out 2>&1"' % (
...
def run_quick_tests(h1,dns,att,mx,zone='example.com'):
    ...
    if mx_answer:
        try:
            ...
            if a:
                out['mx_a']=a
                out['swaks']=_run(h1,f'swaks --to test@{zone} --server {a} --timeout 5 2>&1 || true').strip()
