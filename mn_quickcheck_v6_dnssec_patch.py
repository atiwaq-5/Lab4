"""
Patch helper for mn_quickcheck_v6.py -- add DNSSEC signing on authoritative 'dns' and a validating Unbound on 'h1'.
...
- The function writes files on the remote Mininet host using here-documents. It tries to be idempotent (skips keygen if keys exist).
...
""".format(zone_name=zone_name, ksk_b64=ksk_b64, **locals())
    ...
    # Write Unbound conf file
    unbound_conf_path = '/etc/unbound/unbound.conf.d/example_lab.conf'
    write_cmd = 'bash -lc "cat > %s <<\'UNBOUND\'\n%s\nUNBOUND\'"' % (shlex.quote(unbound_conf_path), unbound_conf)
    _run(h1, write_cmd)
